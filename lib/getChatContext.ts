...
async function buildChatContext(): Promise<string> {
  ...
  const [stories, jobs, partners, partnersBoard, linkedInPosts, articles] = await Promise.all([
...
- When a published article directly supports the answer, use its content and link to its exact Read article URL from PUBLISHED ERETEAM ARTICLES.
