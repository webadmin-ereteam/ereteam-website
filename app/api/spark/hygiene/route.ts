...
import { fetchHubSpotPropertyCatalog, updateHubSpotObjectProperties } from "@/lib/spark/hubspot";
...
const editableProperties = ["country", "vendor_name", "revenue_type", "ereteam_domain"] as const;
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    await Promise.all(parsed.data.records.map((record) => {
      const property = catalogs.get(record.objectType)?.find((item) => item.name === parsed.data.property);
      const value = property?.fieldType === "checkbox" ? parsed.data.values.join(";") : parsed.data.values[0];
      return updateHubSpotObjectProperties(objectTypes[record.objectType], record.id, {
        [parsed.data.property]: value,
      });
    }));
    return NextResponse.json({
      ok: true,
